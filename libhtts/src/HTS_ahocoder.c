...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\*/
/******************************************************************************/
// Versi�n reducida de AHOcoder para hts_engine
...
#define DEFFV 4500.0      // si no hay modelado expl�cito de fvoicing, este ser� el valor de referencia
...
#define F0UV 100.0        // cuando haya que asignar una f0 al ruido, se tomar� este valor
...
/********** Funciones de arm�nicos **********/

unsigned int hanoharms(double fs,double fmax,double f0) {
	// n�mero de arm�nicos de una se�al entre 0 y min{fmax,fs/2}
...
int genharmonics(double *trama,unsigned int N12,unsigned int N23,double fs,double f0,unsigned int K,double *aa,double *pp,double alfa,char wins) {
	// genera un cacho de se�al armonica y lo suma a lo que hubiera
...
int gennoisefromlogspectrum(double *X,unsigned int Lp2,double fs) {
	...
	// ATENCION: X DEBE TRAER TAMA�O 2�Lp2
	// ojo, en realidad basta que me pasen como entrada s�lo las primeras Lp2/2 muestras (incluso sin la primera), pero que los buffers tengan tama�o para todas, claro
...
/********** Funciones de reconstrucci�n sobre CC **********/
...
int cc2waveform(double *x,unsigned int Lx,double fs,unsigned int Lframe,unsigned int Nframes,double **f0s,double **fvs,unsigned int ord,double **CC,double alfa) {
	...
	// calculo los valores maximo y minimo de c0, con los que mapear� la maximum voicing frequency si es caso
	...
	// reservo memoria para las cosillas que ir� sacando
	...
	for (k=0,pm=Lframe,f0ant=0.0;k<Nframes;k++,pm+=Lframe) {
		...
		// primero vamos con la parte del ruido, com�n a todos los frames, incluyendo factor ruido overlap-add
		...
		// remuestreo al tama�o de la fft de sintesis
		...
		if (f0>0.0) {
			...
			// fase m�nima por muestreo de la envolvente complementaria
			...
			// a�ado los armonicos al ruido ya generado
	...
	// libero memoria (recuerda que hab�a una sola reserva para varias matrices y vectores)
...
int gen_ahocoder_waveform(short *s,unsigned int Ls,unsigned int sr,unsigned int Lframe,unsigned int Nframes,double **lf0s,double **fv,unsigned int ord,double alfa,double **CC) {
	...
	// descarte de casos patol�gicos generacion convirtiendo las entradas
...
unsigned int get_ahocoder_waveform_length(unsigned int Lframe,unsigned int Nframes) {
	// devuelve el tama�o que ha de tener la se�al x si viene caracterizada por [Nframes] vectores sacados cada [Lframe] muestras
