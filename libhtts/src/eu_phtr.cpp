...
/********************k+k Orduan k-> [k]*************************************************/
			if(!u.charIsLast(p, URANGE_PAUSE)&&ch2=='k'&& !u.charIsLast(p, URANGE_WORD)&&(!phtsimple)) 
//EVA anadida comprobacion para que no quite una de las k si es entre palabras
			{
				SETPH(p,PHEU_k);
				p2=DEL(p2);
				break;
 ...
 * a todos los verbos bain* la etiqueta de excepcion a la palatalizacion de la n */
	...
	*/
				...
				}//EVA 2016/03/07 faltaba esta llave y no se aplicaba la regla siguiente mas que en las palabras que tenian i+nst o i+nzt		
